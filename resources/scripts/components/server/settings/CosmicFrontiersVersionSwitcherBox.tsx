...
import { fetchVersionsByType, CosmicFrontiersVersion } from '@/api/server/cosmicfrontiers/getCosmicFrontiersVersions';
...
type VersionType = 'stable' | 'nightly';

export default () => {
    const history = useHistory();
    const uuid = ServerContext.useStoreState((state) => state.server.data!.uuid);
    const [activeTab, setActiveTab] = useState<VersionType>('stable');
    const [versions, setVersions] = useState<CosmicFrontiersVersion[]>([]);
    const [selectedVersion, setSelectedVersion] = useState<CosmicFrontiersVersion | null>(null);
    ...
    const handleVersionSelect = (version: CosmicFrontiersVersion) => {
    ...
    const handleProceedToFileSelection = () => {
        ...
        history.push(`/server/${uuid}/settings/cosmic-frontiers-files`, {
            selectedVersion,
        });
    };

    return (
        <TitledGreyBox title={'Cosmic Frontiers Version Switcher'} css={tw`relative`}>
            <SpinnerOverlay visible={loading} />

            <p css={tw`text-sm mb-4 text-neutral-300`}>
                Switch your Cosmic Frontiers server to a different version. Select a version type, choose a version,
                and then select which files to keep during the reinstall.
            </p>
            ...
            <TabContainer>
                ...
                <Tab $active={activeTab === 'nightly'} onClick={() => setActiveTab('nightly')}>
                    Nightlies
                </Tab>
            </TabContainer>
